        ...
        if msg_text.startswith("/unblockmess"):
            ...
            if len(parts) > 1:
                ...
                try:
                    ...
                    if cur.rowcount > 0:
                        reply_text = f"âœ… *Successfully unblocked/cleared custom message for key:*\n`{target_key}`\n\nGagana na ulit ito bilang regular valid key!"
                    else:
                        reply_text = f"âŒ *Key not found in database:* `{target_key}`"
                    
                    cur.close()
                    conn.close()
                except Exception as e:
                    reply_text = f"âŒ *Database Error:* {str(e)}"
                ...
                payload = {
                    "chat_id": chat_id,
                    "text": "âš ï¸ *Usage:* `/unblockmess <iyong_key>`",
                    "parse_mode": "Markdown",
                }
                try:
                    requests.post(url, data=payload, timeout=5)
